...
private:
    void close() {
        ...
        for (std::map<std::string, std::shared_ptr<asio::steady_timer> >::iterator it = tickers_.begin();
             it != tickers_.end(); ++it) {
            it->second->cancel();
        }
        tickers_.clear();
