...
private:
    ...
    void handle_subscribe(const std::string& symbol) {
        ...
        // Capture self in the completion handler to guarantee the Connection outlives the coroutine.
        auto self = shared_from_this();
        asio::co_spawn(strand_, tick_pusher(symbol), [self](std::exception_ptr) {});
    }
    ...
    void enqueue_write(std::vector<unsigned char> frame) {
        ...
        if (!writing_) {
            writing_ = true;
            auto self = shared_from_this();
            asio::co_spawn(strand_, writer(), [self](std::exception_ptr) {});
        }
    }
