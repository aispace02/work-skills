...
```cpp
// Boost >= 1.80 supports the 3-arg overload with a completion token:
asio::spawn(strand,                                   // executor or strand
    ...
    asio::detached);                                  // 3rd arg: completion token (Boost >= 1.80)
// On Boost < 1.80 (e.g. 1.74 on Debian 12), use 2-arg: asio::spawn(strand, [self](...){});
```
    ...
    timer_.async_wait(asio::bind_executor(strand_,
        [this, self](boost::system::error_code ec) {
            if (!ec) {
                ...
                socket_.close(close_ec);        // fired → drop; reset cancels with ec
            }
        }));
}
...
uint32_t len_be_;
std::string body_;
static constexpr std::uint32_t MAX_FRAME_SIZE = 16 * 1024 * 1024; // 16 MB cap
...
void read_frame() {
    ...
    asio::async_read(socket_, asio::buffer(&len_be_, sizeof len_be_),
        asio::bind_executor(strand_, [this, self](boost::system::error_code ec, std::size_t) {
            if (ec) { close(); return; }
            std::uint32_t n = decode_be32(reinterpret_cast<const unsigned char*>(&len_be_));
            if (n == 0 || n > MAX_FRAME_SIZE) {
                close();
                return;
            }
            body_.assign(n, '\0');
            asio::async_read(socket_, asio::buffer(&body_[0], body_.size()), // read exactly N bytes
