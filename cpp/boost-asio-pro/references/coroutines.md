...
public:
    ...
    void send(std::string frame) {
        ...
        if (!writing_) {
            writing_ = true; // Set synchronously BEFORE co_spawn to avoid duplicate write_loop spawns
            co_spawn(socket_.get_executor(),
...
private:
    asio::awaitable<void> write_loop() {
        try {
            ...
            // Socket error or cancellation: safely close socket
            boost::system::error_code ec;
            socket_.close(ec);
        }
        writing_ = false;
...
// Portable big-endian codec (works cross-platform without <arpa/inet.h> or <winsock2.h>):
static inline std::uint32_t decode_be32(const unsigned char* p) {
    return (std::uint32_t(p[0]) << 24) | (std::uint32_t(p[1]) << 16) |
           (std::uint32_t(p[2]) << 8)  |  std::uint32_t(p[3]);
}
static inline void encode_be32(unsigned char* p, std::uint32_t v) {
...
constexpr std::uint32_t MAX_FRAME_SIZE = 16 * 1024 * 1024; // 16 MB cap prevents std::bad_alloc DoS

asio::awaitable<std::string> read_frame(tcp::socket& sock) {
    unsigned char len_buf[4];
    co_await async_read(sock, asio::buffer(len_buf, 4));  // exactly 4 bytes
    std::uint32_t n = decode_be32(len_buf);
    if (n == 0 || n > MAX_FRAME_SIZE) {
        throw std::runtime_error("invalid or excessive frame length");
    }
    std::string body(n, '\0');
...
asio::awaitable<void> write_frame(tcp::socket& sock, std::string_view body) {
    unsigned char len_buf[4];
    encode_be32(len_buf, static_cast<std::uint32_t>(body.size()));
    std::array<asio::const_buffer, 2> bufs{
        asio::buffer(len_buf, 4), asio::buffer(body)};
    co_await async_write(sock, bufs);                     // gather-write header + body atomically
    // len_buf and body must outlive the write — they do here (co_await suspends in-frame).
}
```
