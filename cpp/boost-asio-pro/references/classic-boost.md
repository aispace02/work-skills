...
```
CMake for this range: `cmake_minimum_required(VERSION 3.5)` (Debian 9 ships cmake 3.7), link `Boost::system` if required on older Boost versions (< 1.69; modern Boost provides header-only error codes via `BOOST_ERROR_CODE_HEADER_ONLY`), and use the classic out-of-source build (`mkdir build && cd build && cmake ..`) since `-S`/`-B` need cmake ≥ 3.13.
