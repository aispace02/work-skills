...
所有K线图创建、配置、绘图逻辑必须参考【K线图创建及SetOption参数详解】
所有日K线历史数据、分钟K线数据、回调入参必须参考【K线图数据格式详解】
所有K线图操作接口函数必须参考【K线图操作接口函数详情】
所有K线图配色、样式、指标参数必须参考【K线图配色和样式参数详解】
所有分时图创建、配置、绘图逻辑必须参考【分时图创建及SetOption参数详解】
所有单日分时图数据、多日分时图数据、回调入参必须参考【分时图数据格式详解】
输出代码、参数说明、数据结构示例、问题排查全部以6份文档为唯一标准，不使用非官方自定义写法。
...
- **K线图表数据格式详解**：`references/K线图数据格式详解.md`
  - 适用场景：K线图数据对接、NetworkFilter回调、K线数据格式、分时数据格式等
  - 包含：K线日线全量数据、实时数据、K线分钟数据、流通股本等数据结构定义

- **K线图操作接口函数详情**：`references/K线图操作接口函数详情.md`
  - 适用场景：调用K线图表API、动态修改参数、获取数据、刷新图表等
  - 包含：ChangePeriod,ChangeSymbol 等接口函数说明

- **分时图创建SetOption参数详解**：`references/分时图创建及SetOption参数详解.md`
  - 适用场景：创建分时图、配置参数、指标窗口、十字光标、分时样式等
  - 包含：Type、Windows、Symbol、MinuteLine、MinuteTitle、CorssCursorInfo、NetworkFilter 等全部参数说明

- **分时图数据格式详解**：`references/分时图数据格式详解.md`
  - 适用场景：分时图数据对接、NetworkFilter回调、K线数据格式、分时数据格式等
  - 包含：分时图单日数据、多日数据,实时数据、等数据结构定义
...
1.图形数据对接，统一使用'NetworkFilter'回调函数,
2.NetworkFilter挂载位置：option 顶层属性，随 SetOption 一次性传入
3.外部数据通过NetworkFilter中的callback回调函数返回给图表，图表内部根据不同的图表类型，解析不同的数据结构
4.NetworkFilter中根据不同的数据名称，返回不同的数据结构，具体数据结构请参考官方文档：

- K线图数据结构：`references/K线图数据格式详解.md`
- 分时图数据格式详解：`references/分时图数据格式详解.md`
...
- `references/K线图创建及SetOption参数详解.md` - K线图创建及 SetOption 参数详解
- `references/K线图数据格式详解.md` - K线图数据格式详解
- `references/K线图操作接口函数详情.md` - K线图 API 接口函数说明
- `references/K线图配色和样式参数详解.md` - K线图配色、样式参数说明
- `references/分时图创建及SetOption参数详解.md` - 分时图创建及 SetOption 参数详解
- `references/分时图数据格式详解.md` - 分时图数据格式详解
