...
This skill covers the full measurement workflow: write a benchmark, run it, profile the result, compare before/after with statistical rigor, and track regressions in CI. For optimization patterns to apply after measurement, → See `go-performance` (../go-performance/SKILL.md) skill.
...
- The file still follows Go's one-test-file-per-source-file convention (→ See `go-testing` / `golang-testing` skill), just with the `_bench` suffix marking its narrower purpose.
...
- → See `go-performance` (../go-performance/SKILL.md) skill for optimization patterns to apply after measuring ("if X bottleneck, apply Y")
- → See `go-testing` (../go-testing/SKILL.md) and `golang-testing` (../golang-testing/SKILL.md) for general testing practices and deep test auditing
- → See `go-concurrency` (../go-concurrency/SKILL.md) for mutex contention, channel patterns, and goroutine profiling
