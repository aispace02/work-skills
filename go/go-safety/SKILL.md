...
Maps MUST NOT be accessed concurrently — → see `go-concurrency` (../go-concurrency/SKILL.md) for sync primitives.
...
For integer overflow protection, validate numeric bounds explicitly before narrowing conversions.
...
→ See `go-concurrency` (../go-concurrency/SKILL.md) for goroutine lifecycle and leak prevention.
...
Avoid `init()` functions in favor of explicit `New...()` constructors to keep initialization order deterministic and dependencies testable.

## Enforce with Linters

Many safety pitfalls are caught automatically by linters: `errcheck`, `forcetypeassert`, `nilerr`, `govet`, `staticcheck`. See the `go-linting` (../go-linting/SKILL.md) skill for configuration and usage.
...
| Multiple `init()` with ordering assumptions | `init()` execution order across files is unspecified — use explicit constructors |
| Blocking forever on nil channel | Nil channels block on both send and receive. Always initialize before use |

## Cross-References

- → See `go-concurrency` (../go-concurrency/SKILL.md) for concurrent access patterns and sync primitives
- → See `go-error-handling` (../go-error-handling/SKILL.md) for nil error interface trap and error propagation
- → See `go-linting` (../go-linting/SKILL.md) for static analysis linter rules
- → See `use-modern-go` (../use-modern-go/SKILL.md) for modern Go safe syntax (e.g. Go 1.22+ loop scoping)
