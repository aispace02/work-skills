...
- → See `go-concurrency` (../go-concurrency/SKILL.md) for goroutine cancellation patterns using context
- → See `internal/store` for repository and database operations (QueryContext, ExecContext)
- → See `go-linting` (../go-linting/SKILL.md) for static analysis and linters configuration
