...
* LikeC4 DSL (`.c4`) for a live, renderable model — when the project keeps a LikeC4 project (e.g. a `notes/arch/` directory with a `likec4.config.json`), emit or patch `.c4` files there instead of static diagrams; follow the vendored `likec4-dsl` skill for exact syntax and validate with `npx likec4 validate <project-dir>`
